...
while read -r line; do
    ...
    echo "                    <tr>"
    echo "                        <td>${lineN}</td>"
    echo "                        <td><a class='has-text-info' href='${line}' target='_blank'>${line}</a></td>"
    echo "                        <td><span class='tag is-info'>${code}</span></td>"
    echo "                        <td><span class='tag is-warning'>${encodage}</span></td>"
    echo "                    </tr>"
    lineN=$((lineN + 1))
done < "$chemin"
